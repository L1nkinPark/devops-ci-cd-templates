...
**Amazon Route 53** là một dịch vụ DNS (Domain Name System) được quản lý toàn phần (fully managed), có tính sẵn sàng cao (highly available) và khả năng mở rộng mạnh mẽ của AWS. Tên gọi "Route 53" được lấy cảm hứng từ cổng **Port 53** – cổng mạng chuẩn được sử dụng để xử lý các yêu cầu DNS trên toàn cầu qua giao thức TCP/UDP.

Về cơ bản, DNS hoạt động như một danh bạ điện thoại của Internet, giúp dịch chuyển các tên miền thân thiện với con người (như `h1eudayne.click`) thành các địa chỉ IP dạng số mà máy tính có thể hiểu được (như `192.0.2.1` hoặc `2001:db8::1`).
