...
Sơ đồ mô tả cách phân hoạch 4 subnets trên 2 Availability Zones (`az-1`, `az-2`), tích hợp Internet Gateway, NAT Gateway và S3 Endpoint:

![Hình 2: Network Topology](../../../../images/aws/vpc_lab1_network_topology.png)

> [!NOTE]
> **Lưu ý về nhãn địa chỉ và kích thước trong sơ đồ:** Sơ đồ tham chiếu sử dụng các dải mạng `/24` (như `10.0.1.0/24`, `10.0.2.0/24`, `10.0.3.0/24`, `10.0.4.0/24`) cho mục đích minh họa đơn giản. 
> Tuy nhiên, để đáp ứng đúng yêu cầu của đề bài là **"mỗi subnet chứa ít nhất 1,000 IPs"**, khi thực hiện cấu hình thực tế trên AWS Console chúng ta phải sử dụng block **`/22`** (chứa $1,024$ IPs) với các dải phân hoạch không trùng lặp:
> *   `Public-Subnet-1A`: `10.0.0.0/22` (AZ 1)
> *   `Public-Subnet-1B`: `10.0.4.0/22` (AZ 2)
> *   `Private-Subnet-1A`: `10.0.8.0/22` (AZ 1)
> *   `Private-Subnet-1B`: `10.0.12.0/22` (AZ 2)
