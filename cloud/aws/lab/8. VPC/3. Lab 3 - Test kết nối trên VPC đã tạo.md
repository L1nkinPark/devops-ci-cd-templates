...
#### 3. SSH từ Bastion Host sang Private Instance và kiểm tra NAT Gateway

1. Lấy địa chỉ **Private IP** của `private-instance` từ trang quản trị AWS EC2 (ví dụ: `10.0.8.x`).
2. Trên Terminal của Bastion Host, thực hiện lệnh kết nối SSH sang Private Instance sử dụng key pair vừa tạo:
   ```bash
   ssh -i "test.pem" ec2-user@<YOUR_PRIVATE_IP>
   ```
   *(Thay `<YOUR_PRIVATE_IP>` bằng địa chỉ IP nội bộ của Private Instance, ví dụ: `10.0.8.20`).*
3. Sau khi đăng nhập thành công vào Private Instance, hãy chạy lệnh ping để kiểm tra xem máy chủ trong Private Subnet đã đi ra Internet qua NAT Gateway được cấu hình trước đó chưa:
   ```bash
   ping google.com -c 4
   ```
4. Nếu kết quả ping thành công (các gói tin được truyền nhận thông suốt), chúc mừng bạn đã hoàn tất kiểm tra kết nối cho toàn bộ hệ thống VPC đã thiết kế!
...
8. Thực hiện ping Internet thành công từ Private Instance, chứng minh NAT Gateway hoạt động ổn định.
