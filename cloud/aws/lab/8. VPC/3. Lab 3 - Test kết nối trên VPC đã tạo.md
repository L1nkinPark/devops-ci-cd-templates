...
#### 1. Gán IAM Role truy cập S3 cho Private Instance

Trước khi chạy các lệnh AWS CLI thao tác với dịch vụ S3, EC2 instance của bạn phải có quyền truy cập S3:
1. Tại giao diện **EC2 Dashboard** → Chọn máy ảo `private-instance`.
2. Click **Actions** (hoặc **Actions** ở góc trên bên phải) → Chọn **Security** → **Modify IAM role**.
3. Chọn một IAM Role sẵn có của bạn đã được gắn policy `AmazonS3ReadOnlyAccess` (hoặc quyền tương tự) → Nhấn **Update IAM role** để áp dụng.
...
1. Chạy lệnh liệt kê danh sách các S3 Bucket:
   ...
   aws s3 ls
   ```
2. **Kết quả:** Lệnh AWS CLI vẫn thực hiện thành công và trả về danh sách các S3 bucket hiện có trong tài khoản của bạn (hoặc chạy không bị treo và hiển thị danh sách rỗng nếu tài khoản chưa có bucket nào).
